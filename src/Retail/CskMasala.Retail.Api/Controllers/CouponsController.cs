...
[ApiController]
[Route("api/coupons")]
public class CouponsController(IMediator mediator, IExecutionContext ctx) : ControllerBase
{
    ...
    public async Task<IActionResult> ValidateCoupon(string code, [FromQuery] decimal orderAmount, CancellationToken ct) =>
        Ok(await mediator.Send(new ValidateCouponQuery(code, orderAmount), ct));
    ...
    public async Task<IActionResult> CreateCoupon([FromBody] CreateCouponRequest request, CancellationToken ct)
    {
        ...
        var id = await mediator.Send(new CreateCouponCommand(
